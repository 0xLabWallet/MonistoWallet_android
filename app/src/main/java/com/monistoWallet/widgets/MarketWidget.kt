...
class MarketWidget : GlanceAppWidget() {
    ...
    private fun getDeeplinkUri(item: MarketWidgetItem, type: MarketWidgetType): Uri = when (type) {
        ...
        -> {
            "dexnet://coin-page?uid=${item.uid}".toUri()
        }
        MarketWidgetType.TopNfts -> {
            "dexnet://nft-collection?uid=${item.uid}&blockchainTypeUid=${item.blockchainTypeUid}".toUri()
        }
        MarketWidgetType.TopPlatforms -> {
            "dexnet://top-platforms?uid=${item.uid}&title=${item.title}".toUri()
        }
    }
    ...
    private fun MarketDataValueComponent(
        ...
        when {
            diff != null -> {
                Text(
                    text = com.monistoWallet.core.App.numberFormatter.formatValueAsDiff(com.monistoWallet.modules.market.Value.Percent(diff)),
