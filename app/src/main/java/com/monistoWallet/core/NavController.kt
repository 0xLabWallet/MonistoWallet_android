...
import com.monistoWallet.modules.pin.ConfirmPinFragment
import com.monistoWallet.modules.pin.SetPinFragment
import com.monistoWallet.modules.settings.terms.TermsFragment
...
fun NavController.authorizedAction(action: () -> Unit) {
    if (com.monistoWallet.core.App.pinComponent.isPinSet) {
        slideFromBottomForResult<com.monistoWallet.modules.pin.ConfirmPinFragment.Result>(R.id.confirmPinFragment) {
            if (it.success) {
                action.invoke()
            }
        }
...
fun NavController.navigateWithTermsAccepted(action: () -> Unit) {
    if (!com.monistoWallet.core.App.termsManager.allTermsAccepted) {
