package com.monistoWallet.core.adapters.zcash
...
class ZcashAdapter(
    ...
    private val accountType = (wallet.account.type as? com.monistoWallet.entities.AccountType.Mnemonic) ?: throw UnsupportedAccountException()
    ...
    init {
        val walletInitMode = if (existingWallet) {
            ...
            com.monistoWallet.entities.AccountOrigin.Created -> WalletInitMode.NewWallet
            com.monistoWallet.entities.AccountOrigin.Restored -> WalletInitMode.RestoreWallet
        }

        val birthday = when (wallet.account.origin) {
            com.monistoWallet.entities.AccountOrigin.Created -> runBlocking {
                BlockHeight.ofLatestCheckpoint(context, network)
            }
            com.monistoWallet.entities.AccountOrigin.Restored -> restoreSettings.birthdayHeight
                ?.let { height ->
    ...
    companion object {
        ...
        fun clear(accountId: String) {
            runBlocking {
                Synchronizer.erase(com.monistoWallet.core.App.instance, ZcashNetwork.Mainnet, getValidAliasFromAccountId(accountId))
            }
        }
    }
}
