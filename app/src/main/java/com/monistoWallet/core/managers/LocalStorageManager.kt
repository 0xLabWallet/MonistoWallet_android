...
import com.monistoWallet.core.ILockoutStorage
import com.monistoWallet.core.IMarketStorage
import com.monistoWallet.core.IPinSettingsStorage
...
class LocalStorageManager(
    ...
    private val CARDS_TAB_ENABLED = "cards_tab_enabled"
    private val CRYPTO_WALLET_TAB_ENABLED = "crypto_wallet_tab_enabled"
    ...
    //used only in db migration
    override var bitcoinDerivation: com.monistoWallet.entities.AccountType.Derivation?
        get() {
            val derivationString = preferences.getString(BITCOIN_DERIVATION, null)
            return derivationString?.let { com.monistoWallet.entities.AccountType.Derivation.valueOf(it) }
        }
        set(derivation) {
    ...
    override var currentMarketTab: com.monistoWallet.modules.market.MarketModule.Tab?
        get() = preferences.getString(MARKET_CURRENT_TAB, null)?.let {
            com.monistoWallet.modules.market.MarketModule.Tab.fromString(it)
        }
        set(value) {
    ...
    override var marketFavoritesSortingField: com.monistoWallet.modules.market.SortingField?
        get() = preferences.getString(MARKET_FAVORITES_SORTING_FIELD, null)?.let {
            com.monistoWallet.modules.market.SortingField.fromString(it)
        }
        set(value) {
            preferences.edit().putString(MARKET_FAVORITES_SORTING_FIELD, value?.name).apply()
        }

    override var marketFavoritesMarketField: com.monistoWallet.modules.market.MarketField?
        get() = preferences.getString(MARKET_FAVORITES_MARKET_FIELD, null)?.let {
            com.monistoWallet.modules.market.MarketField.fromString(it)
        }
        set(value) {
    ...
    override var cryptoWalletTabEnabled: Boolean
        get() = preferences.getBoolean(CRYPTO_WALLET_TAB_ENABLED, true)
        set(value) {
            preferences.edit().putBoolean(CRYPTO_WALLET_TAB_ENABLED, value).commit()
            _cryptoWalletTabEnabledFlow.update {
                value
            }
        }
    override var cardsTabEnabled: Boolean
        get() = preferences.getBoolean(CARDS_TAB_ENABLED, true)
        set(value) {
            preferences.edit().putBoolean(CARDS_TAB_ENABLED, value).commit()
            _cardsTabEnabledFlow.update {
                value
            }
        }
    ...
    private val _cryptoWalletTabEnabledFlow = MutableStateFlow(cryptoWalletTabEnabled)
    override val cryptoWalletTabEnabledFlow = _cryptoWalletTabEnabledFlow.asStateFlow()

    private val _cardsTabEnabledFlow = MutableStateFlow(cardsTabEnabled)
    override val cardsTabEnabledFlow = _cardsTabEnabledFlow.asStateFlow()
