...
sealed class CexType : Parcelable {
    ...
    fun serialized() = when (this) {
        is Binance -> listOf("binance", apiKey, secretKey).joinToString(
            dataSeparator
        )
    }
...
sealed class AccountType : Parcelable {
    ...
    enum class Derivation(val value: String) : Parcelable {
        ...
        val addressType: String
            get() = when (this) {
                Derivation.bip44 -> "Legacy"
                Derivation.bip49 -> "SegWit"
                Derivation.bip84 -> "Native SegWit"
                Derivation.bip86 -> "Taproot"
            }
        ...
        val rawName: String
            get() = when (this) {
                ...
                Derivation.bip86 -> "BIP 86"
            }

        val purpose: HDWallet.Purpose
            get() = when (this) {
                Derivation.bip44 -> HDWallet.Purpose.BIP44
                Derivation.bip49 -> HDWallet.Purpose.BIP49
                Derivation.bip84 -> HDWallet.Purpose.BIP84
                Derivation.bip86 -> HDWallet.Purpose.BIP86
            }

        companion object {
            val default = Derivation.bip84
            private val map = values().associateBy(Derivation::value)

            fun fromString(value: String?): Derivation? = Derivation.Companion.map[value]
        }
    }
