...
class TotalBalance(
    ...
    private fun getPrimaryAmount(
        ...
        com.monistoWallet.core.App.numberFormatter.formatFiatShort(it.value, it.currency.symbol, 2)
    }

    private fun getSecondaryAmount(
        totalState: TotalService.State.Visible
    ) = totalState.coinValue?.let {
        "~" + com.monistoWallet.core.App.numberFormatter.formatCoinFull(it.value, it.coin.code, it.decimal)
    }
}
