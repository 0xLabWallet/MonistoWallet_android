package com.monistoWallet.modules.market.filters
...
import com.monistoWallet.modules.market.priceChangeValue
...
class MarketFiltersService(
    ...
    override fun fetchAsync(): Single<List<com.monistoWallet.modules.market.MarketItem>> {
        return getTopMarketList()
            .map { coinMarkets ->
                coinMarkets.map {
                    val coinMarket = it.value

                    com.monistoWallet.modules.market.MarketItem.createFromCoinMarket(coinMarket, baseCurrency, filterPeriod)
                }
            }
    }
